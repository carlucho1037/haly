...
namespace ProcedualWorlds.WaterSystem
{
    ...
    public class PWS_WaterSystem : MonoBehaviour
    {
        ...
        private static Dictionary<Camera, CommandBuffer> _cameras = new Dictionary<Camera, CommandBuffer>();
        ...
        #region Refraction Variables

        #region Private

        private CameraEvent _cameraEventRefraction = CameraEvent.AfterForwardOpaque;
        private Dictionary<Camera, CommandBuffer> _camerasRefraction = new Dictionary<Camera, CommandBuffer>();
        ...
        public void SetReflectionTexture()
        {
            if (m_waterMaterial != null)
            {
        ...
        public void PreRender(Camera i_cam)
        {
            ...
            CommandBuffer cbuf = new CommandBuffer {name = GaiaShaderID._cbufName};
            cbuf.Clear();

            if (m_reflectionCamera.allowHDR)
                cbuf.GetTemporaryRT(GaiaShaderID._grabID, (int)renderSize, (int)renderSize, 0, FilterMode.Bilinear, RenderTextureFormat.DefaultHDR);
            else
                cbuf.GetTemporaryRT(GaiaShaderID._grabID, (int)renderSize, (int)renderSize, 0, FilterMode.Bilinear );
            cbuf.Blit(BuiltinRenderTextureType.CameraTarget, GaiaShaderID._grabID);
            cbuf.SetGlobalTexture(GaiaShaderID.m_cameraOpaqueTexture, GaiaShaderID._grabID);
            i_cam.AddCommandBuffer(_cameraEvent, cbuf );
            _cameras[i_cam] = cbuf;

            //Refraction
            if (_camerasRefraction.ContainsKey(i_cam))
            {
                return;
            }
            CommandBuffer crbuf = new CommandBuffer {name = GaiaShaderID._cbufName};
            crbuf.Clear();

            if (m_reflectionCamera.allowHDR)
                crbuf.GetTemporaryRT(GaiaShaderID._grabID, (int)renderSize, (int)renderSize, 0, FilterMode.Bilinear, RenderTextureFormat.DefaultHDR);
            else
                crbuf.GetTemporaryRT(GaiaShaderID._grabID, (int)renderSize, (int)renderSize, 0, FilterMode.Bilinear );
            crbuf.Blit(BuiltinRenderTextureType.CurrentActive, GaiaShaderID._grabID);
            crbuf.SetGlobalTexture (GaiaShaderID.m_cameraOpaqueTexture, GaiaShaderID._grabID);
            i_cam.AddCommandBuffer (_cameraEventRefraction, crbuf);
            _camerasRefraction[i_cam] = crbuf;
        }
        //Private
        ...
        private void ClearBuffers()
        {
            ...
            foreach (var cam in _camerasRefraction)
            {
                if (cam.Key)
                {
                    cam.Key.RemoveCommandBuffer(_cameraEventRefraction, cam.Value);
                }
            }

            _camerasRefraction.Clear();
        }
