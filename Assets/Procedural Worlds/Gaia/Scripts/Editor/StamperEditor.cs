...
#region AutoSpawnerList
        ...
        {
            //Writing out the render texture directly, would be faster, but not normalized, need to add a normalization shader for this

            //RenderTexture inputTexture = RenderTexture.GetTemporary(terrain.terrainData.heightmapResolution, terrain.terrainData.heightmapResolution, 0, RenderTextureFormat.ARGBFloat);
            //Graphics.Blit(terrain.terrainData.heightmapTexture, inputTexture);
            //ImageProcessing.WriteRenderTexture(GaiaDirectories.GetExportDirectory() + Path.DirectorySeparatorChar + terrain.name + ".exr", inputTexture);
