    ...
    public class GaiaSessionManager : MonoBehaviour
    {
        ...
        public void GetWorldMinMax(ref float minWorldHeight, ref float maxWorldHeight, bool isWorldmap = false)
        {
                ...
                foreach (Terrain t in Terrain.activeTerrains)
                {
                    if (!TerrainHelper.IsWorldMapTerrain(t))
                    {
                        if (m_session.m_terrainMinMaxCache.Find(x => x.guid == AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(t.terrainData))) == null)
                        {
                            ...
                            GetTerrainMinMax(t, ref newMin, ref newMax);
                        }
                    }
                }
            }
            ...
            if (remainingCacheEntries.Count() > 0)
            {
                ...
                if (isWorldmap)
                {
                    GetTerrainMinMax(TerrainLoaderManager.Instance.WorldMapTerrain, ref newMin, ref newMax);
                }
                else
                {
                    if (dynamicLoaded)
                    {
                        ...
                        foreach (TerrainMinMaxHeight tmmh in remainingCacheEntries)
                        {
                            ...
                            if (terrainData != null)
                            {
                                terrainNames.Add(terrainData.name);
                            }
                            
                        }
                        Action<Terrain> act = (t) => GetTerrainMinMax(t, ref newMin, ref newMax);
                        GaiaUtils.CallFunctionOnDynamicLoadedTerrains(act, false, terrainNames, "Evaluating World Height...");
                    }
                    else
                    {
...
//                m_session.m_defaults.m_content = PWCommon3.Utils.ReadAllBytes(m_session.m_defaults.m_fileName);
//                SaveSession();
//#endif
//            }
        }
        ...
        public GaiaSession CreateSession(bool pickupExistingTerrain = false)
        {
            ...
            {
                Action<Terrain> act = (terrain) => GetTerrainMinMax(terrain, ref newMin, ref newMax);
            ...
            else
            {
                ...
                {
                    GetTerrainMinMax(terrain, ref newMin, ref newMax);
                }
            }
            ProgressBar.Clear(ProgressBarPriority.MinMaxHeightCalculation);
#if UNITY_EDITOR
...
//            UnityEngine.Random.InitState(System.Environment.TickCount);
        ...
        public void ForceTerrainMinMaxCalculation(Terrain terrain)
        {
            ...
            GetTerrainMinMax(terrain,ref fake1, ref fake2, true);
            ProgressBar.Clear(ProgressBarPriority.MinMaxHeightCalculation);
        }
        ...
        private void RestoreOriginAndLoadRange()
        {
            ...
            TerrainLoaderManager.Instance.SetLoadingRange(m_loadRangeBackup);
        }
...
#region Terrain Min Max Cache
        private void GetTerrainMinMax(Terrain terrain, ref float min, ref float max, bool forceRecalculate = false)
        {
#if UNITY_EDITOR
            ...
            else
            {
                ...
                GetMinMaxTerrainHeight(terrain, ref newMin, ref newMax);
                if (tmmh != null)
                {
        ...
        private float GetMinMaxTerrainHeight(Terrain terrain, ref float min, ref float max)
        {
            ...
            shader.SetTexture(kernelHandle, "Input", terrain.terrainData.heightmapTexture);
            shader.Dispatch(kernelHandle, terrain.terrainData.heightmapTexture.width / 8, terrain.terrainData.heightmapTexture.height / 8, 1);
            ...
            min = Mathf.Lerp(0, terrain.terrainData.size.y, Mathf.InverseLerp(0, 0.5f, tmm[0].minHeight));
            max = Mathf.Lerp(0, terrain.terrainData.size.y, Mathf.InverseLerp(0, 0.5f, tmm[0].maxHeight));
        ...
        private static bool CheckIfSceneIsSaved(WorldCreationSettings worldCreationSettings)
        {
            bool isSaved = true;
#if UNITY_EDITOR
            ...
            if (worldCreationSettings.m_createInScene && string.IsNullOrEmpty(EditorSceneManager.GetActiveScene().path))
            {
                ...
                if (GaiaUtils.DisplayDialogNoEditor("Scene not saved yet", "You are creating a multi-terrain setup with individual terrain scenes. This requires you to save the scene before proceeding.", "Save Now", "Cancel", false, "Could not setup a multi-terrain world with individual terrain scenes without the scene being saved first!"))
                {
...
#region OPERATION EXECUTION
        private IEnumerator ExecuteCreateWorld(WorldCreationSettings worldCreationSettings)
        {
...
#endif
            TerrainSceneStorage terrainSceneStorage = ScriptableObject.CreateInstance<TerrainSceneStorage>();
            ...
            for (int x = 0; x < worldCreationSettings.m_xTiles; x++)
            {
                for (int z = 0; z < worldCreationSettings.m_zTiles; z++)
                {
                    ...
                    {
#if GAIA_PRO_PRESENT
                        terrainSceneStorage.m_terrainScenes.Add(TerrainSceneCreator.CreateTerrainScene(originalScene, m_session, terrainGO, worldCreationSettings));
#endif
                    }
                    else
                    {
            ...
            TerrainLoaderManager.Instance.m_terrainUnloadMemoryTreshold = gaiaSettings.m_terrainUnloadMemoryTreshold;
                ...
                if (worldCreationSettings.m_createInScene)
                {
                    TerrainLoaderManager.Instance.SetLoadingRange(Mathf.Round((worldCreationSettings.m_tileSize * 1.9f)/2f));
        ...
        private static bool ExecuteClearWorld()
        {
            ...
            if (GaiaUtils.HasDynamicLoadedTerrains())
            {
                ...
                FileUtil.DeleteFileOrDirectory(GaiaDirectories.GetTerrainScenePath(sessionMgr.m_session));
                ...
                if (stamper != null)
                {
                    ...
                    if (GaiaUtils.HasDynamicLoadedTerrains())
                    {
                        ...
                        foreach (TerrainScene terrainScene in TerrainLoaderManager.TerrainScenes)
                        {
                            ...
                            {
                                terrainScene.AddReference(sessionMgr.gameObject);
                            }
                            else
                            {
        ...
        public static void AddTerrainScenesToBuildSettings(List<TerrainScene> allTerrainScenes)
        {
            ...
            {
                EditorBuildSettingsScene sectorSceneSettings = new EditorBuildSettingsScene(terrainScene.m_scenePath, true);
